...
Adatum Corporation 有两层应用程序，它们具有基于 .NET Core 的前端和基于 SQL Server 的后端。Adatum 企业体系结构团队正在探索通过利用 Azure SQL 数据库作为数据层来实现这些应用程序的可能性。考虑到现有 SQL Server 后端的间歇性、不可预测的使用以及对前端应用内置的延迟的相对较高的容忍度，Adatum 正在考虑使用 Azure SQL 数据库的无服务器层。 
...
1. 在你的实验室计算机上，启动 Web 浏览器，导航至 [Azure门户](https://portal.azure.com)，通过提供你将在此实验室中使用的订阅中具有所有者角色的用户帐户的凭据进行登录。

1. 在 Azure 门户中，搜索并选择 **“SQL 数据库”**，然后在 **“SQL 数据库”** 边栏选项卡上，选择 **“+ 添加”**。
...
1. 在 **“服务器”** 下拉列表中，选择 **“新建”**，然后在 **“新建服务器”** 边栏选项卡上，指定以下设置并选择 **“确定”** （保留其他选项为默认值）：
    ...
    | 服务器管理员登录： | **sqladmin** |
    | 密码 | **Pa55w.rd1234** |
    | 地点 | Azure 区域的名称，你可以在其中配置 SQL 数据库 |
    | 允许 Azure 服务访问服务器 | ***选中复选框*** |

1. 在 **“计算 + 存储”** 标签旁，选择 **“配置数据库”** 链接。
...
1. 在 Azure 门户中，搜索并选择 **“SQL 数据库”**，并在 **“SQL 数据库”** 边栏选项卡上，选择代表新创建的条目 **az30303a-db1** Azure SQL 数据库。
...
1. 在 Azure 门户中上一个练习所部署的 Azure SQL 数据库边栏选项卡的 **“设置”** 部分，选择 **“连接字符串”**。
...
1. 在 Azure 门户中，通过在工具栏上选择搜索文本框右侧的图标打开 **“Cloud Shell”** 窗格。

1. 当提示你选择 **“Bash”** 或 **“PowerShell”** 时，选择 **“Bash”**。 

    >**注意**： 如果这是你第一次启动 **Cloud Shell** 且看到 **“你没有装载存储”** 消息，则请选择你在本实验室中使用的订阅，然后选择 **“创建储存”**。 
...
1. 在“Cloud Shell”窗格中，运行以下命令以基于桌面模板为基于 .NET Core 的应用创建新的应用项目文件：

   ```sh
   dotnet new console
   ```

1. 在“Cloud Shell”窗格中，使用内置编辑器打开 **az30303a1.csproj** 文件并通过在标签 `<Project>` 之间添加以下 XML 元素来对其进行修改 ： 
...
1. 在“Cloud Shell”窗格中，运行以下命令来编译新创建的基于 .NET Core 的控制台应用：

   ```sh
   dotnet restore
   ```

1. 在“Cloud Shell”窗格中，运行以下命令以执行新创建的基于 .NET Core 的控制台应用：
...
1. 请注意，执行控制台应用将触发错误。 
    ...
    >**注意**： 这是预料之中的，因为必须明确允许从分配给运行 Cloud Shell 会话的虚拟机的 IP 地址进行连接。


#### 任务 4：配置 Azure SQL 数据库防火墙

1. 在“Cloud Shell”窗格中，运行以下命令以标识运行 Cloud Shell 会话的虚拟机的公共 IP 地址：
...
1. 在 **“防火墙设置”** 边栏选项卡上，设置以下条目并选择 **“保存”**：
    ...
    | --- | --- |
    | 规则名称 | **cloudshell** |
    | 启动 IP | 你在此任务前面确定的 IP 地址 |
    | 结束 IP | 你在此任务前面确定的 IP 地址 |

    >**注意**： 显然，这仅是为了实验室目的，因为在重启 Cloud Shell 会话后，该 IP 地址将更改。
...
1. 请注意，控制台应用这次将成功执行，并且返回的结果与 Azure 门户 SQL 数据库边栏选项卡中查询编辑器中显示的结果相同。 
...
1. 在“Cloud Shell”窗格中，运行以下命令以列出你在本练习中创建的资源组：
   ...
   ```
    ...
    > **注意**： 验证输出结果中是否仅包含你在本实验室中创建的资源组。这些组将在本任务中删除。

1. 在“Cloud Shell”窗格中，运行以下命令以删除你在本实验室中创建的资源组
